...
**已支持**：Springer、Nature、RSC（浏览器 + 机构 IP）、Elsevier/ScienceDirect（官方 API，绕开 Cloudflare）。
...
- ✅ 已端到端验证：框架、Crossref 解析、限速/重试/去重、通行证自动登录（含一次性短信设备验证）、直连绕代理、**Springer / Nature / RSC**（浏览器/IP）、**Elsevier**（官方 API）。
- ⚠️ 反爬硬墙（需令牌/无清晰路径）：Wiley（Cloudflare，可走 TDM 令牌）、ACS（Cloudflare，无公开全文 API）、IOP（Radware）、APS（Cloudflare）；IEEE（Xplore 的 JS viewer，较复杂）。
- 🔜 后续：Wiley TDM 令牌、国内库 CNKI/万方/维普、NSTL 文献传递兜底。
...
| 官方 API/令牌 | Elsevier(10.1016, API key ✅)、Wiley(10.1002, TDM 令牌) | 绕开反爬墙 |
| 反爬硬墙 | ACS(10.1021)、IOP(10.1088)、APS(10.1103) | Cloudflare/Radware，无头爬不动 |
| 较复杂 | IEEE(10.1109) | JS viewer，PDF 取得需额外工作 |
